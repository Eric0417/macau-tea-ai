      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-6 mb-4">
          ...
          {tea && record.confidence && (
            <span className="inline-block bg-emerald-500/15 border border-emerald-400/20 rounded-full px-3 py-1 text-emerald-300 text-sm">
        ...
        <div className="space-y-3">
          {items.map((d, i) => (
            <motion.div key={d.l} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 * i }}
              ...
              <p className="text-white/80 font-medium text-sm mb-1">{d.e} {d.l}</p>
              <p className="text-white/55 text-sm leading-relaxed">{d.v}</p>
            </motion.div>
          ))}
